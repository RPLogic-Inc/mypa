...
async function createSharedTeam(memberIds: string[]) {
  const [adminId, ...others] = memberIds;
  const { teamId } = await createTeamWithAdmin(adminId);
...
describe("POST /contacts/admin/upsert", () => {
  it("rejects requests without relay sync token", async () => {
    const res = await request(app)
      .post("/contacts/admin/upsert")
      .send({ id: USER_A, displayName: "Alice" });

    expect(res.status).toBe(401);
    expect(res.body.error.code).toBe("UNAUTHORIZED");
  });

  it("creates and updates contact with valid relay sync token", async () => {
    const syncToken = process.env.RELAY_SYNC_TOKEN!;

    const createRes = await request(app)
      .post("/contacts/admin/upsert")
      .set("x-relay-sync-token", syncToken)
      .send({
        id: USER_A,
    ...
    expect(createRes.body.data.id).toBe(USER_A);
    expect(createRes.body.data.displayName).toBe("Alice Synced");
    ...
    const updateRes = await request(app)
      ...
      .send({
        id: USER_A,
        displayName: "Alice Synced Updated",
        email: "alice+updated@example.com",
      });

    expect(updateRes.status).toBe(200);
    expect(updateRes.body.data.displayName).toBe("Alice Synced Updated");
    expect(updateRes.body.data.email).toBe("alice+updated@example.com");
  });
});
...
describe("GET /contacts/search", () => {
  it("searches contacts by display name", async () => {
    await createSharedTeam([USER_A, USER_B, USER_C]);
  ...
  it("searches contacts by email", async () => {
    await createSharedTeam([USER_A, USER_B]);
    await registerContact(USER_A, {
      displayName: "Alice",
      email: "alice@example.com",
    });
  ...
  it("does not return contacts from outside shared teams", async () => {
    await createSharedTeam([USER_A, USER_B]);
    await registerContact(USER_A, { displayName: "Alice Team" });
    await registerContact(USER_B, { displayName: "Bob Team" });
    await registerContact(USER_C, { displayName: "Carol External" });
...
describe("GET /contacts/:userId", () => {
  ...
  it("returns 404 for contact outside shared teams", async () => {
    await createSharedTeam([USER_A]);
    await registerContact(USER_B, {
      displayName: "Bob",
      email: "bob@example.com",
