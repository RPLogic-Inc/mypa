...
// ─────────────────────────────────────────────────────────────────────────────
// SPOKE / HUB — hub-and-spoke federation management
// ─────────────────────────────────────────────────────────────────────────────

/** Track which team hubs this personal instance is a spoke for */
export const spokeMemberships = sqliteTable(
  "spoke_memberships",
  {
    id: text("id").primaryKey(),
    hubHost: text("hub_host").notNull(),
    teamId: text("team_id").notNull(),
  ...
  (table) => [
    index("idx_sm_hub").on(table.hubHost),
    index("idx_sm_user").on(table.userId),
  ]
);

/** Track which spokes are connected to this team hub */
export const hubSpokeRegistry = sqliteTable(
  "hub_spoke_registry",
  {
    id: text("id").primaryKey(),
    spokeHost: text("spoke_host").notNull(),
    userId: text("user_id").notNull(),
    role: text("role").notNull().default("member"),
  ...
  (table) => [
    index("idx_hsr_spoke").on(table.spokeHost),
    index("idx_hsr_user").on(table.userId),
    index("idx_hsr_status").on(table.status),
  ]
);
