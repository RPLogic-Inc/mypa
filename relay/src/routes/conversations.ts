...
conversationRoutes.post("/:id/messages", authenticate, async (req, res) => {
  try {
    ...
    // Emit SSE events to all conversation members
    const members = await db
      .select({ userId: conversationMembers.userId })
      .from(conversationMembers)
      .where(eq(conversationMembers.conversationId, conversationId));

    const msgPayload = { tezId, conversationId, senderId: userId, type: body.type };
    for (const { userId: memberId } of members) {
      eventBus.emit(`user:${memberId}`, { type: "new_message", data: msgPayload });
      if (memberId !== userId) {
        eventBus.emit(`user:${memberId}`, { type: "unread_update", data: { reason: "new_message", conversationId, tezId } });
      }
    }
